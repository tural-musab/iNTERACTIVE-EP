...
--
-- Name: create_user_stats(); Type: FUNCTION; Schema: public; Owner: postgres
--
...
20250802205142	\N	remote_commit
20250803021400	{"-- Content tablosu için INSERT policy'si ekle\nCREATE POLICY \\"Authenticated users can insert content\\" ON content\n  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL)","-- Content tablosu için UPDATE policy'si ekle\nCREATE POLICY \\"Content creators can update their content\\" ON content\n  FOR UPDATE USING (auth.uid() IS NOT NULL)"}	add_content_insert_policy
20241219000000	{"-- i-EP MVP Gamification Veritabanı Şeması\n-- Bu script'i Supabase SQL Editor'da çalıştırın\n\n-- 1. Kullanıcı rozetleri tablosu\nCREATE TABLE IF NOT EXISTS user_badges (\n  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,\n  badge_id TEXT NOT NULL,\n  badge_name TEXT NOT NULL,\n  badge_description TEXT,\n  badge_icon TEXT,\n  badge_category TEXT CHECK (badge_category IN ('achievement', 'streak', 'quiz', 'social')),\n  badge_rarity TEXT CHECK (badge_rarity IN ('common', 'rare', 'epic', 'legendary')),\n  points_reward INTEGER DEFAULT 0,\n  awarded_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),\n  UNIQUE(user_id, badge_id)\n)","-- 2. Puan geçmişi tablosu\nCREATE TABLE IF NOT EXISTS point_history (\n  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,\n  points INTEGER NOT NULL,\n  reason TEXT NOT NULL CHECK (reason IN ('quiz_completion', 'streak_bonus', 'achievement', 'daily_login', 'speed_bonus', 'perfect_score')),\n  quiz_id UUID REFERENCES content(id) ON DELETE SET NULL,\n  metadata JSONB,\n  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()\n)","-- 3. Streak takibi tablosu\nCREATE TABLE IF NOT EXISTS user_streaks (\n  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE UNIQUE,\n  current_streak INTEGER DEFAULT 0,\n  longest_streak INTEGER DEFAULT 0,\n  last_activity_date DATE DEFAULT CURRENT_DATE,\n  streak_type TEXT DEFAULT 'daily' CHECK (streak_type IN ('daily', 'weekly', 'quiz')),\n  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()\n)","-- 4. Kullanıcı istatistikleri tablosu\nCREATE TABLE IF NOT EXISTS user_stats (\n  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE UNIQUE,\n  total_quizzes_completed INTEGER DEFAULT 0,\n  total_correct_answers INTEGER DEFAULT 0,\n  total_incorrect_answers INTEGER DEFAULT 0,\n  average_accuracy DECIMAL(5,2) DEFAULT 0,\n  total_time_spent INTEGER DEFAULT 0, -- saniye cinsinden\n  total_points_earned INTEGER DEFAULT 0,\n  favorite_subject TEXT,\n  last_activity TIMESTAMP WITH TIME ZONE DEFAULT NOW(),\n  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),\n  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()\n)","-- 5. Günlük liderlik tablosu (cache için)\nCREATE TABLE IF NOT EXISTS daily_leaderboard (\n  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,\n  username TEXT NOT NULL,\n  avatar_url TEXT,\n  daily_points INTEGER DEFAULT 0,\n  rank INTEGER,\n  date DATE DEFAULT CURRENT_DATE,\n  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),\n  UNIQUE(user_id, date)\n)","-- 6. Analitik events tablosu\nCREATE TABLE IF NOT EXISTS analytics_events (\n  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,\n  event_type TEXT NOT NULL,\n  event_data JSONB,\n  session_id TEXT,\n  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()\n)","-- 7. Users tablosunu güncelle (varsa) - Bu kısım admin yetkisi gerektirdiği için kaldırıldı\n-- Users tablosuna total_xp, level, avatar kolonları manuel olarak eklenmelidir\n-- ALTER TABLE auth.users ADD COLUMN total_xp INTEGER DEFAULT 0;\n-- ALTER TABLE auth.users ADD COLUMN level INTEGER DEFAULT 1;\n-- ALTER TABLE auth.users ADD COLUMN avatar TEXT;\n\n-- 8. Index'ler performans için\nCREATE INDEX IF NOT EXISTS idx_point_history_user_id ON point_history(user_id)","CREATE INDEX IF NOT EXISTS idx_point_history_created_at ON point_history(created_at)","CREATE INDEX IF NOT EXISTS idx_user_badges_user_id ON user_badges(user_id)","CREATE INDEX IF NOT EXISTS idx_user_stats_user_id ON user_stats(user_id)","CREATE INDEX IF NOT EXISTS idx_daily_leaderboard_date ON daily_leaderboard(date)","CREATE INDEX IF NOT EXISTS idx_analytics_events_user_id ON analytics_events(user_id)","CREATE INDEX IF NOT EXISTS idx_analytics_events_created_at ON analytics_events(created_at)","-- 9. RLS (Row Level Security) Policies\nALTER TABLE user_badges ENABLE ROW LEVEL SECURITY","ALTER TABLE point_history ENABLE ROW LEVEL SECURITY","ALTER TABLE user_streaks ENABLE ROW LEVEL SECURITY","ALTER TABLE user_stats ENABLE ROW LEVEL SECURITY","ALTER TABLE daily_leaderboard ENABLE ROW LEVEL SECURITY","ALTER TABLE analytics_events ENABLE ROW LEVEL SECURITY","-- User badges policies\nCREATE POLICY \\"Users can view own badges\\" ON user_badges\n  FOR SELECT USING (auth.uid() = user_id)","CREATE POLICY \\"System can insert badges\\" ON user_badges\n  FOR INSERT WITH CHECK (true)","-- Point history policies\nCREATE POLICY \\"Users can view own point history\\" ON point_history\n  FOR SELECT USING (auth.uid() = user_id)","CREATE POLICY \\"System can insert points\\" ON point_history\n  FOR INSERT WITH CHECK (true)","-- User streaks policies\nCREATE POLICY \\"Users can view own streaks\\" ON user_streaks\n  FOR SELECT USING (auth.uid() = user_id)","CREATE POLICY \\"System can manage streaks\\" ON user_streaks\n  FOR ALL USING (true)","-- User stats policies\nCREATE POLICY \\"Users can view own stats\\" ON user_stats\n  FOR SELECT USING (auth.uid() = user_id)","CREATE POLICY \\"System can manage stats\\" ON user_stats\n  FOR ALL USING (true)","-- Daily leaderboard policies (herkes görebilir)\nCREATE POLICY \\"Everyone can view leaderboard\\" ON daily_leaderboard\n  FOR SELECT USING (true)","CREATE POLICY \\"System can manage leaderboard\\" ON daily_leaderboard\n  FOR ALL USING (true)","-- Analytics events policies\nCREATE POLICY \\"System can insert analytics\\" ON analytics_events\n  FOR INSERT WITH CHECK (true)","CREATE POLICY \\"Users can view own analytics\\" ON analytics_events\n  FOR SELECT USING (auth.uid() = user_id)","-- 10. Functions ve Triggers\n\n-- Kullanıcı stats'ını otomatik oluştur\nCREATE OR REPLACE FUNCTION create_user_stats()\nRETURNS TRIGGER AS $$\nBEGIN\n  INSERT INTO user_stats (user_id)\n  VALUES (NEW.id)\n  ON CONFLICT (user_id) DO NOTHING;\n  \n  INSERT INTO user_streaks (user_id)\n  VALUES (NEW.id)\n  ON CONFLICT (user_id) DO NOTHING;\n  \n  RETURN NEW;\nEND;\n$$ LANGUAGE plpgsql","-- Users tablosuna trigger ekle (eğer yoksa) - Bu kısım admin yetkisi gerektirdiği için kaldırıldı\n-- Trigger'lar manuel olarak eklenmelidir\n-- DROP TRIGGER IF EXISTS create_user_stats_trigger ON auth.users;\n-- CREATE TRIGGER create_user_stats_trigger\n-- AFTER INSERT ON auth.users\n-- FOR EACH ROW\n-- EXECUTE FUNCTION create_user_stats();\n\n-- XP'den level hesaplama fonksiyonu\nCREATE OR REPLACE FUNCTION calculate_level(xp INTEGER)\nRETURNS INTEGER AS $$\nDECLARE\n  level INTEGER := 1;\n  required_xp INTEGER := 0;\nBEGIN\n  WHILE xp >= required_xp LOOP\n    level := level + 1;\n    required_xp := required_xp + (level * 100);\n  END LOOP;\n  RETURN level - 1;\nEND;\n$$ LANGUAGE plpgsql","-- Puan eklendiğinde XP ve level güncelle\nCREATE OR REPLACE FUNCTION update_user_xp_and_level()\nRETURNS TRIGGER AS $$\nBEGIN\n  UPDATE users \n  SET \n    total_xp = total_xp + NEW.points,\n    level = calculate_level(total_xp + NEW.points)\n  WHERE id = NEW.user_id;\n  \n  -- Stats tablosunu da güncelle\n  UPDATE user_stats\n  SET \n    total_points_earned = total_points_earned + NEW.points,\n    updated_at = NOW()\n  WHERE user_id = NEW.user_id;\n  \n  RETURN NEW;\nEND;\n$$ LANGUAGE plpgsql","DROP TRIGGER IF EXISTS update_xp_trigger ON point_history","CREATE TRIGGER update_xp_trigger\nAFTER INSERT ON point_history\nFOR EACH ROW\nEXECUTE FUNCTION update_user_xp_and_level()","-- 11. Varsayılan badge tanımları ekle\nINSERT INTO user_badges (user_id, badge_id, badge_name, badge_description, badge_icon, badge_category, badge_rarity, points_reward)\nVALUES \n  -- Sistem rozetleri (user_id NULL olacak, sadece tanım)\n  (NULL, 'first_quiz', 'İlk Adım', 'İlk quizini tamamladın!', '🎯', 'achievement', 'common', 50),\n  (NULL, 'streak_7', 'Haftalık Tutarlı', '7 gün üst üste aktif oldun!', '🔥', 'streak', 'rare', 100),\n  (NULL, 'perfect_score', 'Mükemmeliyetçi', 'Bir quizde %100 başarı elde ettin!', '⭐', 'quiz', 'epic', 200),\n  (NULL, 'speed_demon', 'Hız Şeytanı', 'Bir quizi 1 dakika içinde tamamladın!', '⚡', 'quiz', 'rare', 150),\n  (NULL, 'quiz_master_10', 'Quiz Ustası', '10 quiz tamamladın!', '🏆', 'achievement', 'rare', 100)\nON CONFLICT DO NOTHING","-- 12. Test verisi (isteğe bağlı)\n-- INSERT INTO point_history (user_id, points, reason, metadata)\n-- SELECT id, 100, 'quiz_completion', '{\\"quiz_id\\": \\"test-quiz-1\\"}'::jsonb\n-- FROM users\n-- LIMIT 1;\n\nGRANT ALL ON ALL TABLES IN SCHEMA public TO authenticated","GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO authenticated","GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA public TO authenticated"}	add_gamification_schema
20250803000001	{"-- Create user_profiles table for gamification data\n-- This is the correct approach for Supabase instead of modifying auth.users\n\n-- 1. User profiles table\nCREATE TABLE IF NOT EXISTS user_profiles (\n  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE UNIQUE,\n  total_xp INTEGER DEFAULT 0,\n  level INTEGER DEFAULT 1,\n  avatar TEXT,\n  display_name TEXT,\n  bio TEXT,\n  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),\n  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()\n)","-- 2. Index for performance\nCREATE INDEX IF NOT EXISTS idx_user_profiles_user_id ON user_profiles(user_id)","CREATE INDEX IF NOT EXISTS idx_user_profiles_total_xp ON user_profiles(total_xp)","CREATE INDEX IF NOT EXISTS idx_user_profiles_level ON user_profiles(level)","-- 3. RLS (Row Level Security) Policies\nALTER TABLE user_profiles ENABLE ROW LEVEL SECURITY","-- Users can view own profile\nCREATE POLICY \\"Users can view own profile\\" ON user_profiles\n  FOR SELECT USING (auth.uid() = user_id)","-- Users can update own profile\nCREATE POLICY \\"Users can update own profile\\" ON user_profiles\n  FOR UPDATE USING (auth.uid() = user_id)","-- System can insert profiles\nCREATE POLICY \\"System can insert profiles\\" ON user_profiles\n  FOR INSERT WITH CHECK (true)","-- 4. Function to automatically create profile when user signs up\nCREATE OR REPLACE FUNCTION create_user_profile()\nRETURNS TRIGGER AS $$\nBEGIN\n  INSERT INTO user_profiles (user_id, total_xp, level)\n  VALUES (NEW.id, 0, 1)\n  ON CONFLICT (user_id) DO NOTHING;\n  \n  RETURN NEW;\nEND;\n$$ LANGUAGE plpgsql","-- 5. Trigger to create profile on user signup\nDROP TRIGGER IF EXISTS create_user_profile_trigger ON auth.users","CREATE TRIGGER create_user_profile_trigger\nAFTER INSERT ON auth.users\nFOR EACH ROW\nEXECUTE FUNCTION create_user_profile()","-- 6. Function to update profile timestamp\nCREATE OR REPLACE FUNCTION update_user_profile_timestamp()\nRETURNS TRIGGER AS $$\nBEGIN\n  NEW.updated_at = NOW();\n  RETURN NEW;\nEND;\n$$ LANGUAGE plpgsql","-- 7. Trigger to update timestamp\nDROP TRIGGER IF EXISTS update_user_profile_timestamp_trigger ON user_profiles","CREATE TRIGGER update_user_profile_timestamp_trigger\nBEFORE UPDATE ON user_profiles\nFOR EACH ROW\nEXECUTE FUNCTION update_user_profile_timestamp()","-- 8. Grant permissions\nGRANT ALL ON user_profiles TO authenticated","GRANT ALL ON user_profiles TO service_role"}	create_user_profiles
20250803000002	{"-- Update user_profiles table to include role, full_name, email, grade fields\n-- and update the trigger to handle options.data from auth.signUp\n\n-- 1. Add new columns to user_profiles table\nALTER TABLE user_profiles \nADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'student',\nADD COLUMN IF NOT EXISTS full_name TEXT,\nADD COLUMN IF NOT EXISTS email TEXT,\nADD COLUMN IF NOT EXISTS grade INTEGER","-- 2. Create index for role-based queries\nCREATE INDEX IF NOT EXISTS idx_user_profiles_role ON user_profiles(role)","CREATE INDEX IF NOT EXISTS idx_user_profiles_email ON user_profiles(email)","-- 3. Update the create_user_profile function to handle options.data\nCREATE OR REPLACE FUNCTION create_user_profile()\nRETURNS TRIGGER AS $$\nDECLARE\n  user_role TEXT := 'student'; -- default role\n  user_full_name TEXT;\n  user_grade INTEGER;\nBEGIN\n  -- Extract data from auth.users.raw_user_meta_data (options.data)\n  IF NEW.raw_user_meta_data IS NOT NULL THEN\n    user_role := COALESCE(NEW.raw_user_meta_data->>'role', 'student');\n    user_full_name := NEW.raw_user_meta_data->>'full_name';\n    user_grade := (NEW.raw_user_meta_data->>'grade')::INTEGER;\n  END IF;\n\n  INSERT INTO user_profiles (\n    user_id, \n    total_xp, \n    level, \n    role, \n    full_name, \n    email, \n    grade\n  )\n  VALUES (\n    NEW.id, \n    0, \n    1, \n    user_role, \n    user_full_name, \n    NEW.email, \n    user_grade\n  )\n  ON CONFLICT (user_id) DO UPDATE SET\n    role = EXCLUDED.role,\n    full_name = EXCLUDED.full_name,\n    email = EXCLUDED.email,\n    grade = EXCLUDED.grade,\n    updated_at = NOW();\n  \n  RETURN NEW;\nEND;\n$$ LANGUAGE plpgsql","-- 4. Update RLS policies to include role-based access\nDROP POLICY IF EXISTS \\"Users can view own profile\\" ON user_profiles","CREATE POLICY \\"Users can view own profile\\" ON user_profiles\n  FOR SELECT USING (auth.uid() = user_id)","DROP POLICY IF EXISTS \\"Users can update own profile\\" ON user_profiles","CREATE POLICY \\"Users can update own profile\\" ON user_profiles\n  FOR UPDATE USING (auth.uid() = user_id)","-- Add policy for role-based queries (for admin purposes)\nCREATE POLICY \\"Users can view profiles by role\\" ON user_profiles\n  FOR SELECT USING (\n    auth.uid() = user_id OR \n    EXISTS (\n      SELECT 1 FROM user_profiles \n      WHERE user_id = auth.uid() AND role IN ('admin', 'superadmin')\n    )\n  )","-- 5. Grant permissions\nGRANT ALL ON user_profiles TO authenticated","GRANT ALL ON user_profiles TO service_role"}	update_user_profiles_schema
20250803000003	{"-- Create quiz_attempts table for tracking quiz completions\n-- This table is used by the award-quiz-points Edge Function\n\nCREATE TABLE IF NOT EXISTS quiz_attempts (\n  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n  student_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,\n  quiz_id UUID REFERENCES content(id) ON DELETE CASCADE,\n  score INTEGER NOT NULL,\n  total_questions INTEGER NOT NULL,\n  time_spent INTEGER NOT NULL, -- milliseconds\n  answers JSONB, -- Store student answers\n  completed_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),\n  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()\n)","-- Indexes for performance\nCREATE INDEX IF NOT EXISTS idx_quiz_attempts_student_id ON quiz_attempts(student_id)","CREATE INDEX IF NOT EXISTS idx_quiz_attempts_quiz_id ON quiz_attempts(quiz_id)","CREATE INDEX IF NOT EXISTS idx_quiz_attempts_completed_at ON quiz_attempts(completed_at)","-- RLS (Row Level Security) Policies\nALTER TABLE quiz_attempts ENABLE ROW LEVEL SECURITY","-- Users can view own quiz attempts\nCREATE POLICY \\"Users can view own quiz attempts\\" ON quiz_attempts\n  FOR SELECT USING (auth.uid() = student_id)","-- Users can insert own quiz attempts\nCREATE POLICY \\"Users can insert own quiz attempts\\" ON quiz_attempts\n  FOR INSERT WITH CHECK (auth.uid() = student_id)","-- Teachers can view quiz attempts for their students (if needed)\nCREATE POLICY \\"Teachers can view student quiz attempts\\" ON quiz_attempts\n  FOR SELECT USING (\n    EXISTS (\n      SELECT 1 FROM user_profiles \n      WHERE user_id = auth.uid() AND role = 'teacher'\n    )\n  )","-- Grant permissions\nGRANT ALL ON quiz_attempts TO authenticated","GRANT ALL ON quiz_attempts TO service_role"}	create_quiz_attempts
20250803000004	{"-- Add creator_id to content table\n-- This links content to the user who created it\n\n-- Add creator_id column\nALTER TABLE content \nADD COLUMN IF NOT EXISTS creator_id UUID REFERENCES auth.users(id) ON DELETE SET NULL","-- Create index for performance\nCREATE INDEX IF NOT EXISTS idx_content_creator_id ON content(creator_id)","-- Update RLS policy to allow content creators to manage their content\nDROP POLICY IF EXISTS \\"Content creators can update their content\\" ON content","CREATE POLICY \\"Content creators can update their content\\" ON content\n  FOR UPDATE USING (\n    auth.uid() = creator_id OR \n    EXISTS (\n      SELECT 1 FROM user_profiles \n      WHERE user_id = auth.uid() AND role IN ('admin', 'superadmin')\n    )\n  )","-- Add policy for content creators to delete their content\nCREATE POLICY \\"Content creators can delete their content\\" ON content\n  FOR DELETE USING (\n    auth.uid() = creator_id OR \n    EXISTS (\n      SELECT 1 FROM user_profiles \n      WHERE user_id = auth.uid() AND role IN ('admin', 'superadmin')\n    )\n  )"}	add_creator_id_to_content
\.
...
--
-- Name: users create_user_stats_trigger; Type: TRIGGER; Schema: auth; Owner: supabase_auth_admin
--

CREATE TRIGGER create_user_stats_trigger AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.create_user_stats();
...
--
-- Name: user_stats System can manage stats; Type: POLICY; Schema: public; Owner: postgres
--
...
--
-- Name: user_streaks System can manage streaks; Type: POLICY; Schema: public; Owner: postgres
--
...
--
-- Name: FUNCTION create_user_stats(); Type: ACL; Schema: public; Owner: postgres
--
